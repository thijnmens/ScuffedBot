from logging import info as logging_info, error as logging_error, warning as logging_warning
...
class user(commands.Cog):
    ...
    async def add(self, ctx, argument=None):
        logging_info(f'Recieved user add {ctx.author.name}')
        if ctx.guild is None:
            logging_info("ctx.guild is None\n----------")
    ...
    async def on_member_remove(self, member):
        ...
        try:
            ...
            logging_info(f"Response: {member.id} has been successfully removed to the database\n----------")
        except Exception as e:
            logging_error(e)
    ...
    async def remove(self, ctx):
        logging_info(f"User remove ran by {ctx.author.name}")
        if ctx.guild is None:
            logging_info("ctx.guild is None\n----------")
            return await ctx.send("Please only use this command within the Scuffed Tourneys server! >w<")
        try:
            ...
            logging_info(f"Response: {ctx.author.id} has been successfully removed to the database\n----------")
        except Exception as e:
            logging_error(e+"\n----------")

    @remove.command(aliases=["username"])
    async def remove_username(self, ctx):
        logging_info(f"Recieved: user remove username {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "username": ctx.author.name
        })
        await ctx.send("I've removed Senpai's username >w<")
        logging_info(f"{ctx.author.name} has removed their username")

    @remove.command(aliases=["steam"])
    async def remove_steam(self, ctx):
        logging_info(f"Recieved: user remove Steam {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "steam": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's Steam >w<")
        logging_info(f"{ctx.author.name} has removed their Steam")

    @remove.command(aliases=["twitch"])
    async def remove_twitch(self, ctx):
        logging_info(f"Recieved: user remove Twitch {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "twitch": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's Twitch >w<")
        logging_info(f"{ctx.author.name} has removed their Twitch")

    @remove.command(aliases=["youtube"])
    async def remove_youtube(self, ctx):
        logging_info(f"Recieved: user remove Youtube {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "youtube": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's Youtube >w<")
        logging_info(f"{ctx.author.name} has removed their Youtube")

    @remove.command(aliases=["twitter"])
    async def remove_twitter(self, ctx):
        logging_info(f"Recieved: user remove Twitter {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "twitter": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's Twitter >w<")
        logging_info(f"{ctx.author.name} has removed their Twitter")

    @remove.command(aliases=["reddit"])
    async def remove_reddit(self, ctx):
        logging_info(f"Recieved: user remove Reddit {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "reddit": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's Reddit >w<")
        logging_info(f"{ctx.author.name} has removed their Reddit")

    @remove.command(aliases=["birthday"])
    async def remove_birthday(self, ctx):
        logging_info(f"Recieved: user remove birthday {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "birthday": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's Birthday >w<")
        logging_info(f"{ctx.author.name} has removed their Birthday")

    @remove.command(aliases=["hmd"])
    async def remove_hmd(self, ctx):
        logging_info(f"Recieved: user remove HMD {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "hmd": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's HMD >w<")
        logging_info(f"{ctx.author.name} has removed their HMD")

    @remove.command(aliases=["pfp"])
    async def remove_pfp(self, ctx):
        logging_info(f"Recieved: user remove pfp {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "pfp": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's pfp >w<")
        logging_info(f"{ctx.author.name} has removed their pfp")

    @remove.command(aliases=["status"])
    async def remove_status(self, ctx):
        logging_info(f"Recieved: user remove status {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "status": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's status >w<")
        logging_info(f"{ctx.author.name} has removed their status")

    @remove.command(aliases=["colour", "color"])
    async def remove_colour(self, ctx):
        logging_info(f"Recieved: user remove colour {ctx.author.name}")
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            "colour": firestore.DELETE_FIELD
        })
        await ctx.send("I've removed Senpai's colour >w<")
        logging_info(f"{ctx.author.name} has removed their colour")
    
    # User update
    @user.group(invoke_without_command=True, )
    async def update(self, ctx):
        logging_info(f"Recieved user update")
        await ctx.send("B-Baka!! You need to tell me what you want to update!!\nUse ``>help update`` to check the valid arguments")
        logging_info("no sub command given\n---------")
    ...
    async def username(self, ctx, *, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their username to {argument}\n----------")
    ...
    async def scoresaber(self, ctx, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their scoresaber to {argument}\n----------")

    @update.command()
    async def steam(self, ctx, argument):
        logging_info(f'Recieved user update steam {ctx.author.name}')
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            'steam': argument})
        await ctx.send("I've updated your Steam, Senpai! >w<")
        logging_info(f"{ctx.author.name} has updated their steam to {argument}\n----------")
    ...
    async def twitch(self, ctx, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their twitch to {argument}\n----------")
    ...
    async def youtube(self, ctx, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their youtube to {argument}\n----------")

    @update.command()
    async def twitter(self, ctx, argument):
        logging_info(f'Recieved user update twitter {ctx.author.name}')
        doc_ref = dab.collection("users").document(str(ctx.author.id))
        doc_ref.update({
            'twitter': argument})
        await ctx.send("I've updated your Twitter, Senpai! >w<")
        logging_info(f"{ctx.author.name} has updated their twitter to {argument}\n----------")
    ...
    async def reddit(self, ctx, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their reddit to {argument}\n----------")
    ...
    async def birthday(self, ctx, argument):
        ...
        if(storer[1] > 12 or storer[1] < 1 or storer[0] > 31 or storer[0] < 1):
            logging_warning("Birthday legitimacy triggered, date and/or month invalid")
        ...
        logging_info(f"{ctx.author.name} has updated their birthday to {argument}\n----------")
    ...
    async def hmd(self, ctx, *, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their status to {self.bot.valid_HMD[pos]}\n----------")
    ...
    async def pfp(self, ctx, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their pfp to {argument}\n----------")
    ...
    async def status(self, ctx, *, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their status to {argument}\n----------")
    ...
    async def colour(self, ctx, argument):
        ...
        logging_info(f"{ctx.author.name} has updated their colour to {argument}\n----------")
